...
return function(Config)
    ...
    if Window.Folder then
        if not isfolder("AnUI/" .. Window.Folder) then
            makefolder("AnUI/" .. Window.Folder)
        end
        if not isfolder("AnUI/" .. Window.Folder .. "/assets") then
            makefolder("AnUI/" .. Window.Folder .. "/assets")
        end
    ...
    if typeof(Window.Background) == "string" and BGVideo then
        ...
        if string.find(BGVideo, "http") then
            ...
            if not isfile(videoPath) then
                ...
                if not success then
                    warn("[ AnUI.Window.Background ] Failed to download video: " .. tostring(result))
                    return
                end
            end
            ...
            if not success then
                warn("[ AnUI.Window.Background ] Failed to load custom asset: " .. tostring(customAsset))
                return
            end
    ...
    Creator.AddSignal(Window.UIElements.Main.Main.Topbar.Left:GetPropertyChangedSignal("AbsoluteSize"), function()
        local LeftWidth = 0
        local RightWidth = Window.UIElements.Main.Main.Topbar.Right.UIListLayout.AbsoluteContentSize.X / Config.AnUI.UIScale
        if WindowTitle and WindowAuthor then
            LeftWidth = math.max(WindowTitle.TextBounds.X / Config.AnUI.UIScale, WindowAuthor.TextBounds.X / Config.AnUI.UIScale)
        else
            LeftWidth = WindowTitle.TextBounds.X / Config.AnUI.UIScale
        end
        if WindowIcon then
            LeftWidth = LeftWidth + (Window.IconSize / Config.AnUI.UIScale) + (Window.UIPadding / Config.AnUI.UIScale) + (4 / Config.AnUI.UIScale)
        end
        Window.UIElements.Main.Main.Topbar.Center.Position = UDim2.new(
            0,
            LeftWidth + (Window.UIPadding / Config.AnUI.UIScale),
            0.5,
            0
        )
        Window.UIElements.Main.Main.Topbar.Center.Size = UDim2.new(
            1,
            - LeftWidth - RightWidth - ((Window.UIPadding * 2) / Config.AnUI.UIScale),
            1,
            0
        )
    end)
    ...
    if not IsVideoBG and Window.Background and typeof(Window.Background) == "table" then
        ...
        Window.UIElements.BackgroundGradient = Creator.NewRoundFrame(Window.UICorner, "Squircle", {
            ...
            ImageTransparency = Window.Transparent and Config.AnUI.TransparencyValue or 0
        }, {
            BackgroundGradient
        })
    end
    ...
    function Window:SetBackgroundTransparency(v)
        ...
        Config.AnUI.TransparencyValue = rounded
    ...
    function Window:Open()
        task.spawn(function()
            ...
            task.spawn(function()
                ...
                Config.AnUI:ToggleAcrylic(true)
            end)
        end)
    end
    function Window:Close()
        ...
        Config.AnUI:ToggleAcrylic(false)
        ...
        function Close:Destroy()
            task.spawn(function()
                ...
                Config.AnUI.DropdownGui:Destroy()
    ...
    function Window:ToggleTransparency(Value)
        ...
        Config.AnUI.Transparent = Value
        
        Window.UIElements.Main.Background.ImageTransparency = Value and Config.AnUI.TransparencyValue or 0
        -- Window.UIElements.Main.Background.ImageLabel.ImageTransparency = Value and Config.AnUI.TransparencyValue or 0
    ...
    function Window:GetUIScale(v)
        return Config.AnUI.UIScale 
    end
    
    function Window:SetUIScale(v)
        Config.AnUI.UIScale = v
        Tween(Config.AnUI.ScreenGui.UIScale, .2, {Scale = v}, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
        return Window
    end
    ...
    local TabModule = TabModuleMain.Init(Window, Config.AnUI, Config.Parent.Parent.ToolTips)
    ...
    function Window:Tab(TabConfig)
        ...
        return TabModule.New(TabConfig, Config.AnUI.UIScale)
    end
    ...
    function Window:Section(SectionConfig)
        return SectionModule.New(SectionConfig, Window.UIElements.SideBar.Frame, Window.Folder, Config.AnUI.UIScale, Window)
    end
    ...
    function Window:Dialog(DialogConfig)
        ...
        local function CheckButtonsOverflow()
            ...
            local totalWidth = ButtonsLayout.AbsoluteContentSize.X / Config.AnUI.UIScale
            local parentWidth = ButtonsContent.AbsoluteSize.X / Config.AnUI.UIScale
            ...
            if totalWidth > parentWidth then
                ...
                if availableSpace > 0 then
                    ...
                    for _, button in ipairs(Buttons) do
                        local buttonWidth = button.AbsoluteSize.X / Config.AnUI.UIScale
