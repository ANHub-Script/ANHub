...
local IconsURL = "https://raw.githubusercontent.com/AdityaNugraha/Icons/main/Main-v2.lua"
...
function Creator.Init(AnUITable)
    AnUI = AnUITable
end
...
function Creator.UpdateTheme(TargetObject, isTween)
    local function ApplyTheme(objData)
        for Property, ColorKey in pairs(objData.Properties or {}) do
            ...
            if value ~= nil then
                if typeof(value) == "Color3" then
                    local gradient = objData.Object:FindFirstChild("AnUIGradient")
                    if gradient then
                        gradient:Destroy()
                    end
